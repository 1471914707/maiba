	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(flag){
			...
			if (Integer.parseInt(request.getParameter("type")) > 3){
				user.setValue(((User)MyDataBase.load(User.class, userId)).getValue()+50);
			}
			if (MyDataBase.update(user)){
				...
				//request.getRequestDispatcher("/user/GetUserDetail?userId="+userId).forward(request, response);
