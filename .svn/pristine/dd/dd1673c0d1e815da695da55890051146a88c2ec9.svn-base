 ...
 */
public class HandleArticleList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HandleArticleList() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		tab.setIndex(index);
		List articleList1= MyDataBase.select(Article.class, "readpower != 2 ORDER BY lastTime desc");
		...
		request.getRequestDispatcher("/login/article-list.jsp").forward(request, response);
	}
